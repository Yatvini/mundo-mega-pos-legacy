...
export async function ensureAttendanceKiosk(){if(!supabase)throw new Error('Supabase no está configurado');const {data,error}=await supabase.rpc('attendance_ensure_kiosk');if(error)throw error;return data?.[0]?.token as string}
