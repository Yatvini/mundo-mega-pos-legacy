...
export async function createPlatformBusiness(input:{name:string;legalName:string;industry:string;adminEmail:string;adminName:string;plan:string;slogan:string;primaryColor:string;accentColor:string}){if(!supabase)throw new Error('Supabase no está configurado');const {data,error}=await supabase.rpc('platform_create_business',{p_name:input.name,p_legal_name:input.legalName,p_industry:input.industry,p_admin_email:input.adminEmail,p_admin_name:input.adminName||null,p_plan:input.plan,p_slogan:input.slogan||null,p_primary_color:input.primaryColor,p_accent_color:input.accentColor});if(error)throw error;return data as string}
export async function updatePlatformBusiness(id:string,changes:{status?:string;plan?:string;name?:string;industry?:string;slogan?:string;modules?:Record<string,boolean>}){if(!supabase)throw new Error('Supabase no está configurado');const {error}=await supabase.rpc('platform_update_business',{p_business_id:id,p_status:changes.status??null,p_plan:changes.plan??null,p_name:changes.name??null,p_industry:changes.industry??null,p_slogan:changes.slogan??null,p_modules:changes.modules??null});if(error)throw error}
export async function resendPlatformInvitation(id:string,email:string,fullName:string){if(!supabase)throw new Error('Supabase no está configurado');const {error}=await supabase.rpc('platform_resend_business_invitation',{p_business_id:id,p_email:email,p_full_name:fullName||null});if(error)throw error}
