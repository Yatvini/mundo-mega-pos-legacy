...
const attendanceAge=(birthDate:string)=>{const b=new Date(`${birthDate}T00:00:00`);const today=new Date();let age=today.getFullYear()-b.getFullYear();const m=today.getMonth()-b.getMonth();if(m<0||(m===0&&today.getDate()<b.getDate()))age--;return Math.max(0,age)}
...
export async function loadAttendancePeopleV2(profile:StoreProfile):Promise<AttendancePersonV2[]>{if(!supabase)throw new Error('Supabase no esta configurado');const {data,error}=await supabase.from('attendance_people_v2').select('id,business_id,branch_id,full_name,birth_date,sex,category_override,active,created_at,branches(name),attendance_qr_tokens_v2(id,active,revoked_at)').eq('business_id',profile.businessId).order('full_name');if(error)throw error;return (data??[]).map((p:any)=>{const token=(p.attendance_qr_tokens_v2??[]).find((t:{id:string;active:boolean;revoked_at:string|null})=>t.active&&!t.revoked_at)??null;return {id:p.id,businessId:p.business_id,branchId:p.branch_id,branchName:(Array.isArray(p.branches)?p.branches[0]?.name:p.branches?.name)??'General',fullName:p.full_name,birthDate:p.birth_date,sex:p.sex,categoryOverride:p.category_override,active:p.active,createdAt:p.created_at,tokenId:token?.id??null,tokenActive:Boolean(token),age:attendanceAge(p.birth_date),category:attendanceCategory(p.birth_date,p.category_override)}})}
...
export async function generateAttendanceQrV2(personId:string){return attendanceQrV2Request<{ok:boolean;tokenId:string|null;token:string;link:string}>({action:'create-token',personId},true)}
