...
export type CashData = { session:null|{id:string;openedAt:string;openingAmount:number}; movements:Array<{id:string;kind:string;amount:number;description:string;createdAt:string;movementCategoryName:string|null}>; payments:Array<{method:string;amount:number}>; refunds?:Array<{method:string;amount:number}> }
...
export type CashClosureMovement = { id:string;createdAt:string;userName:string;paymentMethod:string;kind:'income'|'expense';amount:number;description:string;movementCategoryName?:string|null }
...
export type CashMovementCategory = { id:string;businessId:string;name:string;description:string;active:boolean;createdAt:string;updatedAt:string|null }
...
export type AttendanceSafeDeleteResult = { deleted:boolean;reason:string;movementId?:string;personId?:string;metadata?:Record<string,unknown> }
...
type CashMovementRow = { id:string;kind:string;amount:number|string;description:string;created_at:string;movement_category_name_snapshot:string|null }
...
type CashMovementCategoryRow = { id:string;business_id:string;name:string;description:string|null;is_active:boolean;created_at:string;updated_at:string|null }
...
export async function loadCash(profile: StoreProfile): Promise<CashData> {
  ...
  const [{data:movements,error:me},{data:payments,error:pe},{data:returns,error:re}]=await Promise.all([
    supabase.from('cash_movements').select('id,kind,amount,description,created_at,movement_category_name_snapshot').eq('session_id',session.id).eq('status','active').order('created_at',{ascending:false}),
...
const mapCashMovementCategory=(row:CashMovementCategoryRow):CashMovementCategory=>({id:row.id,businessId:row.business_id,name:row.name,description:row.description??'',active:Boolean(row.is_active),createdAt:row.created_at,updatedAt:row.updated_at})
export async function listCashMovementCategories():Promise<CashMovementCategory[]>{if(!supabase)throw new Error('Supabase no esta configurado');const {data,error}=await supabase.rpc('admin_list_cash_movement_categories');if(error)throw error;return (data??[]).map((row:CashMovementCategoryRow)=>mapCashMovementCategory(row))}
export async function createCashMovementCategory(name:string,description=''):Promise<CashMovementCategory>{if(!supabase)throw new Error('Supabase no esta configurado');const {data,error}=await supabase.rpc('admin_create_cash_movement_category',{p_name:name,p_description:description||null});if(error)throw error;return mapCashMovementCategory((data??[])[0] as CashMovementCategoryRow)}
export async function updateCashMovementCategory(categoryId:string,name:string,description:string,isActive:boolean):Promise<CashMovementCategory>{if(!supabase)throw new Error('Supabase no esta configurado');const {data,error}=await supabase.rpc('admin_update_cash_movement_category',{p_category_id:categoryId,p_name:name,p_description:description||null,p_is_active:isActive});if(error)throw error;return mapCashMovementCategory((data??[])[0] as CashMovementCategoryRow)}
export async function listActiveCashMovementCategories():Promise<CashMovementCategory[]>{if(!supabase)throw new Error('Supabase no esta configurado');const {data,error}=await supabase.rpc('list_active_cash_movement_categories');if(error)throw error;return (data??[]).map((row:CashMovementCategoryRow)=>mapCashMovementCategory(row))}
export async function addCashMovement(_profile:StoreProfile,sessionId:string,kind:'income'|'expense',amount:number,description:string,movementCategoryId?:string|null){if(!supabase)throw new Error('Supabase no está configurado');const normalizedCategoryId=movementCategoryId?.trim()||null;const {error}=await supabase.rpc('record_cash_movement',{p_session_id:sessionId,p_kind:kind,p_amount:amount,p_description:description,p_movement_category_id:normalizedCategoryId});if(error)throw error}
...
export async function deleteAttendanceMovementV3(movementId:string):Promise<AttendanceSafeDeleteResult>{const result=await attendanceV3Request<{ok:boolean;deleted:boolean;reason:string;movementId:string}>({action:'delete-movement',movementId},true);return {deleted:Boolean(result.deleted),reason:result.reason??'',movementId:result.movementId??movementId}}
export async function deleteAttendancePersonQr(personId:string):Promise<AttendanceSafeDeleteResult>{const result=await attendanceV3Request<{ok:boolean;deleted:boolean;reason:string;personId:string}>({action:'delete-person-qr',personId},true);return {deleted:Boolean(result.deleted),reason:result.reason??'',personId:result.personId??personId}}
export async function forceDeleteAttendanceMovementV3(movementId:string):Promise<AttendanceSafeDeleteResult>{const result=await attendanceV3Request<{ok:boolean;deleted:boolean;reason:string;movementId:string;metadata?:Record<string,unknown>}>({action:'force-delete-movement',movementId},true);return {deleted:Boolean(result.deleted),reason:result.reason??'',movementId:result.movementId??movementId,metadata:result.metadata??{}}}
export async function forceDeleteAttendancePersonQr(personId:string):Promise<AttendanceSafeDeleteResult>{const result=await attendanceV3Request<{ok:boolean;deleted:boolean;reason:string;personId:string;metadata?:Record<string,unknown>}>({action:'force-delete-person-qr',personId},true);return {deleted:Boolean(result.deleted),reason:result.reason??'',personId:result.personId??personId,metadata:result.metadata??{}}}
