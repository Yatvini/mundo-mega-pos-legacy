...
export async function markAttendance(token:string,employeeId:string,categoryId:string,note='',geo?:{latitude:number;longitude:number;accuracy?:number}){if(!supabase)throw new Error('Supabase no estÃ¡ configurado');const {data,error}=await supabase.rpc('attendance_mark',{p_token:token,p_employee_id:employeeId,p_category_id:categoryId,p_note:note||null,p_latitude:geo?.latitude??null,p_longitude:geo?.longitude??null,p_accuracy_m:geo?.accuracy?Math.round(geo.accuracy):null});if(error)throw error;return data as {id:string;markedAt:string;employeeName:string;categoryName:string;status:string;alertReason?:string;distanceM?:number}}
export async function markAttendanceManual(employeeId:string,categoryId:string,markedAt:string,note=''){if(!supabase)throw new Error('Supabase no estÃ¡ configurado');const {data,error}=await supabase.rpc('attendance_manual_mark',{p_employee_id:employeeId,p_category_id:categoryId,p_marked_at:markedAt,p_note:note||null});if(error)throw error;return data as {id:string;markedAt:string;employeeName:string;categoryName:string;status:string}}
